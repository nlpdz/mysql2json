...
def TableToJson():
    try:
        ...
        conn = pymysql.Connect(host='localhost', user='root', passwd='123456', db='zhihu_masters', port=3306, charset='utf8')
        # 2.创建mysql数据库游标对象 cursor
        # cursor对象支持的方法有execute(sql语句),fetchone(),fetchmany(size),fetchall(),rowcount,close()
        cur = conn.cursor()
        # 3.编写sql
        sql = "SELECT id,bio,description,answer_count,zhihu_url,public_answer_count,user_id,user_name,gender,avatar_url,question_price,follower_count FROM xinlixue"
        # 4.执行sql命令
        ...
        for row in data:
            ...
            result['answer_count'] = row[3]
            result['zhihu_url'] = row[4]
            result['public_answer_count'] = row[5]
            # result['user_id'] = row[6]

            # result['avatar_url'] = row[9]
            result['question_price'] = row[10]
            # result['follower_count'] = row[11]


            f = open(r'D:\xinlixue.txt', 'a+')
            values = result.values()
            try:
                f.write(str(values)[11:])
                f.write('\n')
            except:
                pass
